...
LOGO() {
    ...
    echo -e " ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───"
    echo -e " ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───"
    echo -e "    │    ${YELLOW}Copyright${FONT} (C)${GRAY}https://t.me/amantubilah$NC   │"
    echo -e "    └───────────────────────────────────────────────┘"
...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To anggunvpn Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}TekiroVpn ${NC}${YELLOW}(${NC} ${green}Jabar Putra ${NC}${YELLOW})${NC}"
echo -e "       © SMVPN & anggunvpn ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/arismaramar/izinkansaya/main/ip"
...
checking_sc() {
  ...
  if [[ $(date -d "$date_list" +%s) -lt $(date -d "$useexp" +%s) ]]; then
    ...
    echo -e "\033[42m          ANGGUNvpn AUTOSCRIPT PREMIUM          \033[0m"
    echo -e "\033[1;93m────────────────────────────────────────────\033[0m"
    echo -e ""
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/arismaramar/izinkansaya/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/arismaramar/izinkansaya/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/arismaramar/izinkansaya/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/arismaramar/v3/main/"
...
wget https://raw.githubusercontent.com/arismaramar/v3/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
wget -q -O /usr/local/lynz/udp-mini "${REPO}limit/udp-mini"
...
# Ganti Banner
wget -O /etc/lynz.txt "${REPO}limit/issue.net"
